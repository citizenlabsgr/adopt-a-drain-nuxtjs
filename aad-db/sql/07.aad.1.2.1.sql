...
-- TODO 1.2.1: Change 1_2_0 to 1_2_1
-- TODO 1.2.1: Create add_base schema
-- TODO 1.2.1: Move adopt_a_drain table to aad_base schema
-- TODO 1.2.1: stop insert of duplicate adoptee
-- TODO 1.2.1: add reg_data to adopter insert
-- TODO 1.2.1: add reg_data to adoptee insert
-- TODO 1.2.1: add reg_ata to signin
...
---------------
CREATE SCHEMA if not exists aad_version_1_2_1;
...
grant usage on schema aad_version_1_2_1 to guest_aad;
grant usage on schema aad_version_1_2_1 to editor_aad;
grant usage on schema aad_version_1_2_1 to event_logger_role;
...
SET search_path TO aad_version_1_2_1, aad_base, public;

----------------
-- TYPE: Create Types
----------------
CREATE TYPE aad_version_1_2_1.woden_token AS (
  woden text
);
CREATE TYPE aad_version_1_2_1.jwt_token AS (
  token text
);
...
CREATE OR REPLACE FUNCTION aad_version_1_2_1.signin_validate(form JSONB) RETURNS JSONB
AS $$
  BEGIN
    -- confirm all required attributes are in form
    if not(form ? 'type' and form ? 'name' and form ? 'password') then
        return '{"status":"400", "msg":"Bad Request, missing one or more form attributes.","type":"signin"}'::JSONB;
    end if;
    -- validate attribute values
    if not(form ->> 'type' = 'signin') then
        return '{"status":"400", "msg":"Bad Request type value.","type":"signin"}'::JSONB;
    end if;
    -- proper password
    if not (exists(select regexp_matches(form ->> 'password', '^(?=.{8,}$)(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*\W).*$') )) then
        return '{"status":"400","msg": "Bad Request, bad password.","type":"signin"}'::JSONB;
    end if;
    -- proper name ... name
    if not( exists( select regexp_matches(form ->> 'name', '[a-z\-_0-9]+@[a-z]+\.[a-z]+') ) ) then
      return '{"status":"400", "msg":"Bad Request, bad name.","type":"signin"}'::JSONB;
    end if;
...
$$ LANGUAGE plpgsql;
-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_2_1.signin_validate(JSONB) to guest_aad;


------------
-- FUNCTION: Create aad_version_1_2_1.signin(form JSON)
------------
/*
CREATE OR REPLACE FUNCTION aad_version_1_2_1.signin(form JSON) RETURNS JSONB
AS $$
...
grant EXECUTE on FUNCTION aad_version_1_2_1.signin(JSON) to guest_aad;
*/
...
AS $$
  ...
  BEGIN
    ...
    if not(_jwt_role = 'guest_aad') then
      _validation := '{"status":"401", "msg":"UnAuthorized","type":"signin"}'::JSONB;
    ...
    -- confirm all required attributes are in form
    -- validate attribute values
    _validation := aad_version_1_2_1.signin_validate(_form);
    ...
    IF _fr is NULL THEN
      -- login failure
      _validation :=  '{"status":"403", "msg":"Forbidden","type":"signin"}'::JSONB || _form;
      PERFORM aad_base.event_logger(_validation);
...
CREATE OR REPLACE FUNCTION aad_version_1_2_1.adopter(form JSON) RETURNS JSONB
AS $$
  Declare rc jsonb;
  ...
  BEGIN
    ...
    -- confirm all required attributes are in form
    -- validate attribute values
    _validation := aad_version_1_2_1.adopter_validate(_form);
    if _validation ->> 'status' != '200' then
        PERFORM aad_base.event_logger(_validation);
        return _validation;
    end if;
...
$$ LANGUAGE plpgsql;
-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_2_1.adopter(JSON) to guest_aad; -- upsert
grant EXECUTE on FUNCTION aad_version_1_2_1.adopter(JSON) to editor_aad; -- upsert

-----------------
-- FUNCTION: Create adopter_validate(form JSONB)
-----------------
CREATE OR REPLACE FUNCTION aad_version_1_2_1.adopter_validate(form JSONB) RETURNS JSONB
AS $$
  ...
  BEGIN
    ...
    if not(form ->> 'type' = 'adopter') then
       return '{"status":"400", "msg":"Bad Request type value.","type":"adopter"}'::JSONB;
    end if;
...
$$ LANGUAGE plpgsql;
-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_2_1.adopter_validate(JSONB) to guest_aad; -- upsert
grant EXECUTE on FUNCTION aad_version_1_2_1.adopter_validate(JSONB) to editor_aad; -- upsert
...
CREATE OR REPLACE FUNCTION aad_version_1_2_1.adopter(id TEXT) RETURNS JSONB
AS $$
  Select reg_form-'password' as adopter from aad_base.adopt_a_drain  where reg_pk=id and reg_sk='adopter';
$$ LANGUAGE sql;
-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_2_1.adopter(JSON) to editor_aad; -- select
...
*/
CREATE OR REPLACE FUNCTION aad_version_1_2_1.adoptee_validate(form JSONB) RETURNS JSONB
AS $$

  BEGIN
    -- confirm all required attributes are in form
    if not(form ? 'lat' and form ? 'lon' and form ? 'drain_id' and form ? 'adopter_key' ) then
       return format('{"status":"400","msg":"Bad Request, missing one or more form attributes", "type":"adoptee", "form":%s}', form)::JSONB;
    end if;
    -- validate attribute values
    if not(form ->> 'type' = 'adoptee') then
       return '{"status":"400", "msg":"Bad Request type value.", "type":"adoptee"}'::JSONB;
    end if;
...
$$ LANGUAGE plpgsql;
-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_2_1.adoptee_validate(JSONB) to editor_aad;
...
CREATE OR REPLACE FUNCTION aad_version_1_2_1.adoptee(form JSON) RETURNS JSONB
AS $$
  Declare rc jsonb;
  Declare _model_user JSONB;
  Declare _form JSONB;
...
grant EXECUTE on FUNCTION aad_version_1_2_1.adoptee(JSON) to editor_aad; -- C

--------------------
-- FUNCTION: Create adoptee(id TEXT)
--------------------
CREATE OR REPLACE FUNCTION aad_version_1_2_1.adoptee(id TEXT) RETURNS JSONB
AS $$
  Select reg_form from aad_base.adopt_a_drain  where reg_pk=id and reg_sk='adoptee';
$$ LANGUAGE sql;

-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_2_1.adoptee(TEXT) to editor_aad; -- C

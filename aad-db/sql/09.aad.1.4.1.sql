...
---------------
CREATE SCHEMA if not exists aad_version_1_4_1;
...
grant usage on schema aad_version_1_4_1 to guest_aad;
grant usage on schema aad_version_1_4_1 to editor_aad;
grant usage on schema aad_version_1_4_1 to event_logger_role;

---------------
-- SCHEMA: Set Schema Path
---------------

SET search_path TO aad_version_1_4_1, aad_base, public;

----------------
-- TYPE: Create Types
----------------
CREATE TYPE aad_version_1_4_1.woden_token AS (
  woden text
);
CREATE TYPE aad_version_1_4_1.jwt_token AS (
  token text
);

------------
-- FUNCTION: Create aad_version_1_4_1.signin(form JSON)
------------

CREATE OR REPLACE FUNCTION aad_version_1_4_1.signin(form JSON) RETURNS JSONB
AS $$
...
$$ LANGUAGE plpgsql;
-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_4_1.signin(JSON) to guest_aad;

-----------------
-- FUNCTION: Create adopter(form JSON)
-----------------

CREATE OR REPLACE FUNCTION aad_version_1_4_1.adopter(form JSON) RETURNS JSONB
AS $$
  Declare rc jsonb;
...
$$ LANGUAGE plpgsql;
-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_4_1.adopter(JSON) to guest_aad; -- upsert
grant EXECUTE on FUNCTION aad_version_1_4_1.adopter(JSON) to editor_aad; -- upsert
...
CREATE OR REPLACE FUNCTION aad_version_1_4_1.adopter(id TEXT) RETURNS JSONB
AS $$
  Select reg_form-'password' as adopter from aad_base.adopt_a_drain  where reg_pk=id and reg_sk='adopter';
$$ LANGUAGE sql;
-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_4_1.adopter(TEXT) to editor_aad; -- select
...
CREATE OR REPLACE FUNCTION aad_version_1_4_1.adoptee(form JSON) RETURNS JSONB
AS $$
  Declare rc jsonb;
  Declare _model_user JSONB;
  Declare _form JSONB;
...
grant EXECUTE on FUNCTION aad_version_1_4_1.adoptee(JSON) to editor_aad; -- C

--------------------
-- FUNCTION: Create adoptee(id TEXT)
--------------------
CREATE OR REPLACE FUNCTION aad_version_1_4_1.adoptee(id TEXT) RETURNS JSONB
AS $$
  Select reg_form from aad_base.adopt_a_drain  where reg_pk=id and reg_sk='adoptee';
$$ LANGUAGE sql;

-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_4_1.adoptee(TEXT) to editor_aad; -- C
--------------------
-- FUNCTION: Create adoptees(JSON)
--------------------
-- find in boundary
CREATE OR REPLACE FUNCTION aad_version_1_4_1.adoptees(bounds JSON) RETURNS TABLE (adoptee jsonb)
AS $$
Declare _validation JSONB;
...
$$ LANGUAGE plpgsql;
-- GRANT: Grant Execute
grant EXECUTE on FUNCTION aad_version_1_4_1.adoptees(JSON) to editor_aad; -- C
grant EXECUTE on FUNCTION aad_version_1_4_1.adoptees(JSON) to guest_aad; -- C

...
SET search_path TO aad_version_1_4_1, public;
...
BEGIN;
  ...
  SELECT is (
    ...
    'event_logger - insert test  1_4_1'::TEXT
  );
...
BEGIN;
  ...
  SELECT ok (
    aad_version_1_4_1.adopter('{
      "name":  "me@someplace.com",
      "password": "a1A!aaaa"
      }'::JSON
    )::JSONB ->> 'status' = '200','adopter - new adopter 1_4_1'
  );

  -- duplicate adopter
-- PREPARE duplicate_adopter AS aad_version_1_4_1.adopter('{"name":  "me@someplace.com", "password": "a1A!aaaa"}'::JSON);
PREPARE new_adopter as select aad_version_1_4_1.adopter('{"name":  "me@someplace.com", "password": "a1A!aaaa"}'::JSON);
...
SELECT ok (
  aad_version_1_4_1.signin('{
    "name":  "me@someplace.com",
    "password": "a1A!aaaa"
    }'
  )::JSONB ? 'token','signin - 200 insert 1_4_1'
);
...
SELECT is (
  ...
  '{"msg": "OK", "status": 200}'::JSONB,
  'adoptee - insert test 1_4_1'::TEXT
);
SELECT is (
  aad_version_1_4_1.adoptee( '{
    "name":"some opt name",
...
PREPARE new_adoptee AS select aad_version_1_4_1.adoptee( '{
  "name":"some opt name",
  "drain_id":"GR_40089457",
  "lat":42.96265175640001,
  "lon":-85.6676956307}'::JSON
